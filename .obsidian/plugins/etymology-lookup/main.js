...
// lib/etymo-js/lib/util.ts
function getIdFromPath(path) {
  var _a2;
  return (_a2 = path.match(/\d+$/)) == null ? void 0 : _a2[0];
}

// lib/etymo-js/lib/cheerioOps.ts
function getEntriesFromSearch(html3) {
  const $2 = load(html3);
  const entries = [];
  const entriesNodes = $2('[class^="word--"]');
  entriesNodes.each((_i, el) => {
    const term = $2(el).find('[class^="word__name--"]').text();
    const def = $2(el).find('[class^="word__defination--"]').text();
    const path = $2(el).find("a")[0].attribs.href;
    const id = getIdFromPath(path);
    const entry = { term, def, path, id };
    entries.push(entry);
  });
  return entries;
}

// lib/etymo-js/lib/get.ts
var import_obsidian = require("obsidian");
async function getHtml(url) {
  const headers = { "Content-Type": "text/html" };
  const res = await (0, import_obsidian.requestUrl)({ url, headers });
  return res.text;
}

// lib/etymo-js/constants.ts
var BASE_URL = "https://www.etymonline.com";

// lib/etymo-js/index.ts
var Etymo = class {
  async search(term) {
    const url = this._buildSearchUrl(term);
    const html3 = await getHtml(url);
    const entries = getEntriesFromSearch(html3);
    return entries;
  }
...
// util/displayEntries.ts
var import_obsidian2 = require("obsidian");
function displayEntries(entries, contentEl) {
  if (!entries.length) {
...
// util/ellipsis.ts
function ellipsis(str, length = 10) {
...
// main.ts
var etymo = new Etymo();
var EtymologyLookupModal = class extends import_obsidian3.Modal {
...
var EtymologyLookupPlugin = class extends import_obsidian3.Plugin {
  async onload() {
    console.log("loading Etymology Lookup plugin");
    this.addRibbonIcon("sprout", "Etymology Lookup", (event) => {
  ...
  async lookup(selection) {
    const modal = new EtymologyLookupModal(this.app, selection);
    modal.open();
  }
};
function getCurrentSelectedText(app) {
  var _a2;
  const editor = (_a2 = app.workspace.activeEditor) == null ? void 0 : _a2.editor;
